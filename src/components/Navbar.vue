<template>
  <div class="bg-white">
        ...
        <div class="fixed inset-0 z-40 flex">
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
            <DialogPanel class="relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl">
                ...
                <div class="border-b border-gray-200">
                  <TabList class="-mb-px flex space-x-8 px-4">
                    <Tab as="template" v-for="category in navigation.categories" :key="category.name" v-slot="{ selected }">
                      <button :class="[selected ? 'border-green-700 text-green-700' : 'border-transparent text-gray-900', 'flex-1 border-b-2 px-1 py-4 text-base font-medium whitespace-nowrap outline-none']">{{ category.name }}</button>
                    </Tab>
                  </TabList>
                </div>
                  ...
                  <TabPanel v-for="category in navigation.categories" :key="category.name" class="space-y-10 px-4 pt-10 pb-8">
                    <div class="grid grid-cols-2 gap-x-4">
                      <div v-for="item in category.featured" :key="item.name" class="group relative text-sm">
                        ...
                        <RouterLink :to="item.href" class="mt-6 block font-medium text-gray-900">
                    ...
                    <div v-for="section in category.sections" :key="section.name">
                      ...
                      <ul role="list" :aria-labelledby="`${category.id}-${section.id}-heading-mobile`" class="mt-6 flex flex-col space-y-6">
                        <li v-for="item in section.items" :key="item.name" class="flow-root">
                          <RouterLink :to="item.href" class="-m-2 block p-2 text-gray-500">{{ item.name }}</RouterLink>
                        </li>
                      </ul>
                    </div>
                  </TabPanel>
              ...
              <div class="space-y-6 border-t border-gray-200 px-4 py-6">
                <div class="flow-root">
                  <RouterLink to="/signin" class="-m-2 block p-2 font-medium text-gray-900">Sign in</RouterLink>
                </div>
                <div class="flow-root">
                  <RouterLink to="/signup" class="-m-2 block p-2 font-medium text-gray-900">Create account</RouterLink>
                </div>
              </div>

              <div class="border-t border-gray-200 px-4 py-6">
                <a href="#" class="-m-2 flex items-center p-2">
                  <img :src="gh" alt="" class="block h-auto w-5 shrink-0" />
                  <span class="ml-3 block text-base font-medium text-gray-900">GHA</span>
    ...
    <header class="relative bg-white">
      <p class="flex h-10 items-center justify-center bg-green-700 px-4 text-sm font-medium text-white sm:px-6 lg:px-8 text-center">Get free delivery on every order from August 15 - September 15🎉</p>
      ...
      <nav aria-label="Top" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="border-b border-gray-200">
          <div class="flex h-16 items-center">
            ...
            <!-- Logo -->
            <RouterLink to="/" class="ml-4 flex lg:ml-0">
              <a href="#">
              ...
              <div class="flex h-full space-x-8">
                <Popover v-for="category in navigation.categories" :key="category.name" class="flex" v-slot="{ open }">
                  <div class="relative flex">
                    <PopoverButton :class="[open ? 'text-green-700' : 'text-gray-700 hover:text-gray-800', 'relative flex items-center justify-center text-sm font-medium transition-colors duration-200 ease-out']">
                      {{ category.name }}
                      <span :class="[open ? 'bg-green-700' : '', 'absolute inset-x-0 -bottom-px z-30 h-0.5 transition duration-200 ease-out']" aria-hidden="true" />
                    </PopoverButton>
                  </div>
                ...
                <RouterLink  v-for="page in navigation.pages" :key="page.name" :to="page.href" class="flex items-center text-sm font-medium text-gray-700 hover:text-gray-800">{{ page.name }}</RouterLink>
              </div>
            </PopoverGroup>

            <div class="ml-auto flex items-center">
              <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                <RouterLink to="/signin" class="text-sm font-medium text-gray-700 hover:text-gray-800">Sign in</RouterLink>
                <span class="h-6 w-px bg-gray-200" aria-hidden="true" />
                <RouterLink to="/signup" class="text-sm font-medium text-gray-700 hover:text-gray-800">Create account</RouterLink>
              </div>

              <div class="hidden lg:ml-8 lg:flex">
                <a href="#" class="flex items-center text-gray-700 hover:text-gray-800">
                 <img :src="gh" alt="" class="h-4">
                  <span class="ml-3 block text-sm font-medium">GHA</span>
                  <span class="sr-only">, change currency</span>
                </a>
              </div>
              ...
              <!-- Cart -->
              <div  class="ml-4 flow-root lg:ml-6" @click="showCart = true">
